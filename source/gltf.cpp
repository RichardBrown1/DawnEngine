...
namespace {
	void processMeshData(fastgltf::Asset& asset, glm::f32mat4x4& transform, uint32_t meshIndex) {
...
fastgltf::Asset getAsset(std::string gltfFilePath) {
	...
	auto wholeGltf = parser.loadGltf(gltfFile.get(), "models", fastgltf::Options::LoadExternalBuffers);
...
// void processSceneNodes(fastgltf::Asset& asset) {
...
//				processMeshData(asset, matrix, static_cast<uint32_t>(node.meshIndex.value()));
//				return;
//			}
